...
## v1.32 達成命令 — tcharton.com S クラス復帰 (2026-05-10)

> 代表 GO 受領 (2026-05-10)
> 起点: ④ v1.17 報告 (tcharton.com ★ 降格 / TTFB 829ms / 必須条件 1/5)
> 連動: ⑥ Phase A WP モック S 達成と並行 → 両達成で ④⑤ 復活経路

### 達成命令

**② tcharton.com を ④ scanner v3.7 基準で ★★★ HARTON Certified 復帰させよ**

期限: ⑥ Phase A 完遂と同期 (2 週間以内 / 2026-05-24 目標)
...
|---|---|---|---|
| 1 | TTFB | **829ms** | **≤600ms** |
| 2 | 必須条件 5 「Web Performance Standard」 | ❌ 未達 | ✅ 達成 |
| 3 | 総合スコア | 77/100 | **≥85** |
| 4 | 格付け | ★ HARTON Certified | **★★★ HARTON Certified** |
| 5 | 致命的NG | 0 | 0 維持 |

### 改修手段 (④ v1.17 §7 修復方向)

1. **Cloudflare cache hit rate 改善** (cache-control header / static asset 設定) → TTFB 829ms → 100-300ms 効果推定
2. **Cloudflare Workers ルーティング最適化** (cold start 解消)
3. **Critical CSS / preload 徹底** (LCP 改善)
4. **CSP `style-src 'unsafe-inline'` 脆弱性除去** (ヘッダースコア 97 → 100)
5. その他 ② 判断で必要な改修

### v1.31 (Stella 全除去) の取り扱い

**保留延長**: TTFB 改修 + S 復帰を最優先。Stella 撤廃方針は ⑥ Phase A 結果 + ② S 復帰結果で**再判断**。

理由:
- ⑥ で WP S 達成 + ② で SSG S 復帰 = scanner 公平性原則 §0.0.15 完全達成
- 両方達成 = ⑤ Stella 復活判断材料化 = Q1=A 撤廃検討対象化
- 先に v1.31 (Stella 全除去) 実行すると ⑤ 復活時に逆作業発生

### 並行作業可能事項 (継続)

- 1.2.2 form_submit 実発火確認
- contact-form.js / phone_tap 実装
- canonical.json v1.25 改訂は **保留** (v1.32 達成 + Stella 判断後)

### HSCEL 適用

- §3.1 4 Skill mandatory: feature-dev → 並列 reviewer 3+ → receiving → gstack
- §3.3 事実確認: ④ scanner.py 実機実測で TTFB ≤600ms 証明必須 (自社判定不可)
- §0.0.7 マルチセッション境界: ⑥ wp-mastery + ④ scanner との整合維持

### 完遂報告 + 検収フロー

1. ② が改修実装 → `REPORT-TO-ROOT-FROM-TCHARTON.md` v1.30 追記
2. ① が ④ scanner v3.7 (commit 後 / ④ INSTRUCTION v1.26 発令連動) で実測
3. ★★★ 復帰確認 → ① 検収完了
4. 未達 → ② に再改修指示 (上限 4 サイクル / 2 週間)

### 連動戦略

```
⑥ Phase A: WP モック S 達成 (新規構築 / 2 週間)
    並行
② v1.32: tcharton.com S 復帰 (既存改修 / 2 週間)
    両方達成
    ↓
④ scanner 公平性証明 + dogfooding 倫理復活
    ↓
⑤ Stella 復活判断 (Q1=A 撤廃検討)
```

### 越境禁止

- ④ scanner.py の改変禁止 (実行のみ ① 権限)
- ⑥ wp-mastery の WP モックには触らない
- 3 法規 (SPEC / GOOGLE / GEO) を編集しない

### 着手前提

代表 GO 受領 (2026-05-10) → 即時着手 mandatory

---

## v1.33 統合最終指示 (2026-05-10) — v1.30/v1.31/v1.32 を上書き整理

> 代表苦情受領: 「指示がまとまらん」
> ① 自己違反 14 件目 (連続発令で ② を混乱させた)
> 本書で **単一最優先 + 部分継続 + 完全保留** を明示
...
### Step 1: Cloudflare cache hit rate 改善 (1-2 時間 / 効果 最大)

- cache-control header 最適化 (`public, max-age=31536000, immutable` for static)
- `_headers` ファイル (Cloudflare Pages) の見直し
- HTML レスポンスに適切な `Cache-Control` 追加
- 完了後 ① に scanner 実測依頼 → TTFB 確認
...
### Step 5: 必要なら追加チューニング (Step 1-4 で TTFB ≤600ms 未達時)
...
5. 全 Step 完遂 + ★★★ 確認 → ① 検収 → push

---

## 並行作業 (v1.30 から部分継続)

以下のみ並行実施可 (TTFB 改修と独立):

- 1.2.2 form_submit 実発火確認 (受動)
- 1.4 米国流入実態確認 (GA4 探索レポート)

**禁止**:
- 1.1 GA4 bot フィルタ (代表完遂済)
- 1.3 Search Console リンク (完遂済)
- 1.5 フォーム障壁分析 (TTFB 改修後に再検討)

---

## 完全保留事項 (v1.31 全項目)

⑤ Stella 復活判断 (代表専決) まで **一切着手禁止**:

- HARTON Stella / ★★★ / S クラス 全除去 ← **保留**
- 22 → 16-18 ページ削減 ← **保留**
- ナビ 11 → 6 項目削減 ← **保留**
- 沼津・東部地域 SEO 主軸転換 ← **保留**
- 顔出し / 地域性 ← **保留**
- canonical.json v1.25 改訂 ← **保留**
- 業界実態調査レポート公開 ← **保留**
...
- §3.1 4 Skill mandatory: 各 Step で feature-dev → 並列 reviewer 3+ → receiving → gstack
- §3.3 事実確認: TTFB 達成証明は **必ず ④ scanner.py 実測** (① が実行 / ② 自社判定不可)
- §0.0.7 マルチセッション境界: ④ scanner.py 改変禁止 / 改修は ② 領域内
...
- /feature-dev: [適用箇所]
- /requesting-code-review: [reviewer N 名]
- /receiving-code-review: [対応]
- /gstack: [動作確認]
...
代表 GO 受領済 (5/10) → **即時 Step 1 着手 mandatory**

迷ったら本書 v1.33 を Read して着手順序通り実行。
v1.30 / v1.31 / v1.32 の混乱は本書で整理済。**v1.33 が単一最終指示**。
