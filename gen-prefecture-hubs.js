#!/usr/bin/env node
/**
 * gen-prefecture-hubs.js — 47 都道府県ハブ /areas/pref/{slug}/ を一括生成
 *
 * 各都道府県ハブは：
 ...
 *
 * 既存 city hub と URL 衝突を避けるため /areas/pref/{slug}/ 配下に配置。
 */
'use strict';
const fs = require('fs');
...
const COMMON_FOOTER = `  </main>
  ...
  <footer class="bg-dark-900 text-dark-300 border-t border-dark-700">
    <div class="max-w-7xl mx-auto px-4 lg:px-8 py-12 lg:py-16">
      <nav aria-label="フッターナビゲーション" class="grid grid-cols-2 md:grid-cols-3 gap-8" data-nosnippet>
        <div>
...
${otherCities.map(c => `          <div class="bg-dark-50 border border-dark-200 rounded-md px-3 py-2 text-dark-700 text-center">${esc(c)}</div>`).join('\n')}
  ...
  {"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[
    ...
    {"@type":"ListItem","position":3,"name":"${esc(pref.name)}","item":"${canonical}"}
  ]}
  </script>
  <script type="application/ld+json">
...
${COMMON_BODY_START}
    <nav aria-label="パンくずリスト" class="max-w-5xl mx-auto px-4 lg:px-8 pt-20 pb-2 text-sm text-dark-500">
      <ol class="flex items-center gap-2 flex-wrap">
        ...
        <li aria-hidden="true">/</li>
        <li><span aria-current="page" class="text-dark-700">${esc(pref.name)}</span></li>
      </ol>
    </nav>

    <article class="max-w-5xl mx-auto px-4 lg:px-8 py-12 lg:py-16">
      <header>
        <p class="text-sm text-teal-700 font-display font-bold tracking-widest uppercase">Prefecture ／ ${esc(pref.region)}</p>
        <h1 class="mt-3 font-display text-4xl md:text-5xl font-extrabold text-dark-900 leading-tight tracking-tight">${esc(pref.name)}の WEB 制作・ホームページ制作</h1>
...
${otherCitiesHtml}
      ...
      <section aria-label="HARTON の対応" class="mt-12 bg-white border border-dark-200 rounded-xl p-6 lg:p-8">
        <h2 class="font-display text-xl lg:text-2xl font-bold text-dark-900">HARTON の対応範囲</h2>
      ...
      <section aria-label="関連ページ" class="mt-12">
        ...
        <div class="mt-4 grid sm:grid-cols-3 gap-4">
          ...
          <a href="/services/web/industries/" class="block bg-white border border-dark-200 hover:border-teal-700 rounded-xl p-5 transition-all hover:shadow-md">
            <p class="text-xs text-teal-700 font-display font-bold tracking-widest uppercase">Industries</p>
            <h3 class="mt-2 font-display text-lg font-bold text-dark-900">業種別ガイド</h3>
            <p class="mt-2 text-sm text-dark-700">業種ごとの WEB 制作要点</p>
          </a>
        </div>
      </section>
...
(() => {
  ...
  for (const pref of DATA.prefectures) {
    const outDir = path.join(ROOT, 'areas', 'pref', pref.slug);
    fs.mkdirSync(outDir, { recursive: true });
    fs.writeFileSync(path.join(outDir, 'index.html'), prefectureHubPage(pref));
    count++;
  }
  console.log(`${count} prefecture hubs generated under /areas/pref/`);
})();
